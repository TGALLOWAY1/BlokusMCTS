...
- **Mid-battery eval time budget**: the head-to-head deadline is checked before each `(arena, seed)` sub-battery (not just between candidates), so a single candidate's battery can no longer blow the wall-clock budget and get the job cancelled mid-evaluation — `training/evaluation/head_to_head.py`

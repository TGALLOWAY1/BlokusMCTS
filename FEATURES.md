...
- Champion v2 (validated): `champion_minimal` config promoted to `champion_v2` after Night-1 reset run (2026-06-14), gate PASS: Δμ=7.39 vs champion_v1, pairwise 38-22, TrueSkill μ=39.38 — `data/champion_registry.json` (current_version: v2), `arena_runs/night1_champion_reset/20260614_135743_ca1ba49d/`
- Night-1 champion reset run: 60-game arena (champion_minimal vs champion_v1 vs pool_peer_500ms vs pool_heuristic) at 500 ms, round-robin seats, 769 se_* snapshot rows, Wilson-95 LB=0.819 vs heuristic — `arena_runs/night1_champion_reset/20260614_135743_ca1ba49d/summary.json`
- Overnight training roadmap 2026-05-14: 7-night plan, Night 1 complete (2026-06-14, v2 promoted), Night 2 next (L8 workers×budget Pareto sweep) — `docs/overnight_training_roadmap_2026-05-14.md`
- Champion progression doc: single source of truth for the agent storyline — `champion_v2` is the current validated champion (promoted Night 1), `champion_v1` is a failed full-stack candidate, peer gap (30-30 tie) is the next target — `docs/CHAMPION_PROGRESSION.md`

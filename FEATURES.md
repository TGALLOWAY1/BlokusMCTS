...
- Champion self-improvement loop: persistent champion vs randomized challenger pool (13 MCTS variants), snapshot accumulation in `data/champion_loop_state.json`, evaluator recalibration (per-phase linear regression, every REFIT_INTERVAL generations), TrueSkill tracking, and Markdown progress report — `scripts/champion_loop.py`, `data/champion_loop_state.json`, `data/champion_loop_progress.md`
- Champion arena (primary improvement driver): champion vs randomized 14-agent pool (baselines, time-budget sweep, deploy proxies, RAVE/rollout/widening variants), persistent TrueSkill across runs, snapshot accumulation into `data/champion_snapshots.csv`, inline evaluator weight refitting every N runs (configurable via `--refit-interval`), auto-promotion when challenger conservative-TrueSkill surpasses champion, per-run detailed markdown reports, cumulative progress report with TrueSkill trend and human-level milestones — `scripts/champion_arena.py`, `data/champion_state.json`, `data/champion_reports/`, `data/champion_progress.md`

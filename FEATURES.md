...
- Champion self-improvement loop: persistent champion vs randomized 3-challenger pool (heuristic + checkpoint + MCTS variant), snapshot-driven evaluator recalibration every 3 generations via per-phase linear regression, auto-promotion when a challenger variant beats champion in >60% h2h over ≥6 comparisons, human benchmark target (μ=35) displayed in leaderboard, cross-session TrueSkill persistence, pairwise h2h rates in generation records and markdown progress report — `scripts/champion_loop.py`, `data/champion_state.json`, `data/champion_progress.md`

...
- Champion League: registry-integrated self-improvement loop — champion (from `data/champion_registry.json`) vs randomised per-round pool of 3 MCTS variants; persistent cross-session TrueSkill seeded from `games.jsonl` per-game scores; cumulative snapshot parquet accumulation; per-phase linear regression calibration triggered by row threshold; automatic champion promotion to new registry version on calibration; detailed log in `data/champion_league_log.json`; human-target µ tracking — `scripts/champion_league.py`, `data/champion_trueskill.json`, `data/champion_league_snapshots.parquet`, `data/champion_league_log.json`

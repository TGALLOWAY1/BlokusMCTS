...
- Champion arena: continuous self-improvement loop — champion vs randomized pool (14+ named strategies) with persistent cross-session TrueSkill, snapshot accumulation to master CSV, per-phase evaluator weight re-fitting every N runs (sklearn linear regression), auto-promotion when challenger's conservative rating surpasses champion, human-competitiveness tracking vs heuristic proxy, detailed per-run markdown reports, and `--generations N` batch mode — `scripts/champion_arena.py`, `data/champion_arena_state.json`, `data/champion_arena_snapshots.csv`, `data/champion_reports/`

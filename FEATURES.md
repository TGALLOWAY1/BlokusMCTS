...
- Stable evaluation + promotion: fixed, versioned benchmark pool (`benchmark_v2`: heuristic, random, the fixed-parameter MCTS anchors `baseline_mcts_fast`/`baseline_mcts_strong`, and best historical champion when one exists) with fixed seeds, head-to-head pooled battery, and a statistical promotion gate (beats champion H2H + positive Elo/TrueSkill Δ + conservative 6-gate + min games/seeds). Candidate arenas pair the weak anchors in one arena and the MCTS anchors in the other so candidates are never measured only beside heuristic/random before the first promotion — `training/evaluation/{benchmark_pool,head_to_head,promotion_gate,rating_analysis}.py`
...
- **Fixed-champion measurement-drift reporting** (audit follow-up #5): the comparison record carries a `champion_status` block, and when the champion has never been promoted (`last_promoted_generation` is null) the agent under measurement is byte-for-byte fixed, so Elo movement is rating *drift*, not a strength change. The approach-comparison markdown retitles the Elo trajectory as "⚠️ fixed-champion measurement drift", the email subject annotates the delta as `(…, drift)`, and the email verdict reports a neutral `➖ STEADY — fixed-champion measurement drift` instead of `✅ GOING WELL` on a noise-driven Elo rise (unless a candidate actually promoted this run) — `training/evaluation/report.py` (`build_comparison_record`, `render_markdown`), `training/email_summary.py` (`champion_is_fixed`, `overall_verdict`, `build_subject`)
- **Two-stage promotion** (opt-in, `--two-stage-promotion`; audit follow-up #4): the cheap 20-game gate is the *screen* that picks a leading candidate; before promoting, ONLY that candidate is re-evaluated over a larger confirmation sample (`EVAL_CONFIRM_TOTAL_GAMES = 60`) against the same fixed pool and must clear the gate again at the higher floor, so a lucky short run can no longer promote on its own. Confirmation games fold into the champion's rating timeline; the outcome is recorded in `last_approach_comparison.confirmation` and rendered in the report — `training/evaluation/head_to_head.py` (`confirm_winner`, `confirm_games_per_arena`), `training/nightly_run.py` (`run_approaches`)
- **History schema versioning** (audit follow-up #3): every `history.jsonl` row is stamped with `schema_version`/`kind` (`approach_comparison` vs `legacy_generation`); `state_store.classify_history_row` classifies older unstamped rows structurally so the state-audit diagnostic validates only comparable rows together and no longer flags legacy rows as "malformed" or "missing result-like fields" — `training/state_store.py`, `training/nightly_run.py`, `training/diagnostics/audit_training_state.py`
- **Per-game play-quality diagnostics** (audit follow-up #2): each arena game record carries a `play_quality` block — average/min/max legal moves per turn, pass rate, invalid-move count, game length, board occupancy, piece usage by size (overall and per agent), and the final-score distribution (min/max/spread) — computed at record time with negligible overhead (values already on hand each turn) — `analytics/tournament/arena_runner.py` (`run_single_game`, `_piece_size`)

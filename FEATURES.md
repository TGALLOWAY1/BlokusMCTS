...
- Unified champion self-improvement loop: persistent champion vs 4-tier stratified challenger pool (16 agents: baselines, time-budget sweep, feature-variant MCTS, deploy-difficulty proxies); per-phase evaluator regression on accumulated snapshot rows; auto-promotion of winning Tier-2 variants; human-target progress indicator (μ-3σ vs calibrated estimate); versioned checkpoints; per-generation reports in `data/champion_reports/`; cumulative TrueSkill trend in `data/champion_progress.md` — `scripts/champion_loop.py`, `data/champion_state.json`
- Champion arena (alternative single-run tool): champion vs explicit named pool with persistent TrueSkill, snapshot collection, auto-promotion, and per-run markdown reports — `scripts/champion_arena.py`, `data/champion_arena_state.json`, `data/champion_reports/`

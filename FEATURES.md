...
- Champion self-improvement loop: persistent champion vs 3-tier randomized challenger pool (22 agents across TIER_WEAK/TIER_MEDIUM/TIER_STRONG), snapshot-driven evaluator recalibration every 3 generations, auto-promotion when a challenger's conservative TrueSkill surpasses champion's with ≥20 games, heuristic "human-proxy" tracked as a fixed rating anchor with gap reported per generation, calibrated phase weights loaded from `data/layer6_calibrated_weights.json` on startup, Markdown progress report — `scripts/champion_loop.py`, `data/champion_state.json`, `data/champion_progress.md`

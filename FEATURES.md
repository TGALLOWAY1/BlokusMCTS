...
- Champion self-improvement loop: persistent champion competes each generation against a randomized 3-agent pool (heuristic anchor + recent champion checkpoint + random MCTS variant from 13-strategy catalog); board-state snapshots accumulated in `data/champion_snapshots.csv`; per-phase linear regression refits evaluator weights every 3 generations; TrueSkill ratings and champion params persisted in `data/champion_state.json`; per-generation Markdown progress report at `data/champion_progress.md` — `scripts/champion_loop.py`

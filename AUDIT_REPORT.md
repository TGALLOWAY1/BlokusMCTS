...
core configs and `data/` registry/calibration files.
...
### 3.3 Crippled, never-replaced champion (critical) ✅ fixed
`gen0` champion = random rollouts + depth-5 cutoff + 0.5 iter/ms + RAVE
(see §1.2). Replaced via the new promotion workflow with a clean strong
baseline (heuristic rollouts, no cutoff, calibrated 10 iter/ms budget, no
RAVE/minimax) after a seeded head-to-head gauntlet — see §5.

...
| `arena_runs/night1_champion_reset/20260614_135743_ca1ba49d/` | `champion_minimal`, `champion_v1`, `pool_peer_500ms`, `pool_heuristic` | 60 | `champion_minimal` beats `champion_v1`: pairwise 38-22, Δμ=7.39 (mu 39.38 vs 32.00). Wilson-95 LB vs heuristic = 0.819. CM ties peer 30-30. Gate PASS → promoted to `champion_v2`. | `candidate_validation` (gate PASS) | TrueSkill anchors: CM μ=39.38, peer μ=36.15, v1 μ=32.00, heuristic μ=−6.95; 769 `se_*` snapshot rows; gate evidence for v2 promotion. | Headline "beats humans" WR claim — Wilson-95 LB=0.819 is promising but sample is 60 games at one seed. Night 4 (4-seed × 60-game) is required before claiming ≥65% LB at scale. CM ties peer (30-30): no decisive advantage yet. |
